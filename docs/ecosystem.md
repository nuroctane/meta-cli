...
| **Cua** | [trycua/cua](https://github.com/trycua/cua) computer-use driver (`cua-driver`) — full-desktop automation via MCP + CLI. Auto-installed on single-shot install **without** the elevated autostart daemon (`-NoAutoStart`); start it on demand with `cua-driver serve`, or wire its MCP with `cua-driver mcp-config` |
