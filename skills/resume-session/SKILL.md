...
description: >
  ...
  Claude Code, Codex, Cursor, NurCLI, and Grok Build. Use with resume-claude,
  resume-codex, resume-cursor, resume-nur, or resume-grok when the user wants
...
| `session_reader.py` | `list` / `show` for `claude` · `codex` · `cursor` · `nur` · `grok` |
...
| `nur` | **resume-nur** | `~/.nur/sessions/*.json` (NurCLI) |
...
The reader tool id is always the short token: `claude` | `codex` | `cursor` | `nur` | `grok`.
...
| continue my Meta session / resume meta | resume-nur |
